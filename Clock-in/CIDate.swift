...
extension NSDate {
    ...
    func intersectionWithEntry(entry:CIModelEntry) -> NSTimeInterval {
        let targetStart = self.roundToDay()
        let targetEnd = targetStart.advancedByDays(1)
        
        let entryStart = entry.startDate
        let entryEnd = entry.startDate.dateByAddingTimeInterval(entry.time)
        
        if targetStart.compare(entryEnd) == .OrderedDescending || entryStart.compare(targetEnd) == .OrderedDescending {
            return 0
        }
        
        let laterStart = (entryStart.compare(targetStart) == .OrderedDescending) ? entryStart : targetStart
        let earlierEnd = (entryEnd.compare(targetEnd) == .OrderedAscending) ? entryEnd : targetEnd
        
        return earlierEnd.timeIntervalSinceDate(laterStart)
    }
}