...
struct CIModelTestItemProfile {
    ...
    init(profileName:String) {
        if profileName == "Work" {
            ...
            self.init(minStartOffset: (60*60*12), maxStartOffset: (60*60*21), minClockDuration: (60*20), maxClockDuration: (60*60*2), daysToIterate: 40, probabilitiesForClocksPerWeekday: [0.3, 0.3, 0.2, 0.2], probabilitiesForClocksPerWeekend: [0.2, 0.5, 0.3])
        }
        else if profileName == "Coding" {
            self.init(minStartOffset: (60*60*10), maxStartOffset: (60*60*20), minClockDuration: (60*45), maxClockDuration: (60*60*2), daysToIterate: 120, probabilitiesForClocksPerWeekday: [0.2, 0.6, 0.1, 0.1], probabilitiesForClocksPerWeekend: [0, 0.3, 0.2, 0.3, 0.2])
        }
        else {
